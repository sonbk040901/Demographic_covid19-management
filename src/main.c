...
{
    char ch;
    int len = strlen(str) + y - 1;
    int deflen = len;
    int count = 0;
    do
    {
        ch = mvwgetch(xyz, x, len);
        if (ch == 27 || ch == 9 || ch == 10)
        {
            break;
        }
...
void BOXlogin() // in ra window function
{
    ...
    id = derwin(login, 3, 50, 1, 1);
    pass = derwin(login, 3, 50, 4, 1);
    box(id, ACS_VLINE, ACS_HLINE);
    box(pass, ACS_VLINE, ACS_HLINE);

    wattron(login, A_STANDOUT);
    wattrset(id, COLOR_PAIR(14));
    wattrset(pass, COLOR_PAIR(14));
    mvwprintw(id, 1, 1, "%s", "Tai Khoan:");
    mvwprintw(pass, 1, 1, "%s", "Mat Khau:");
    wattroff(id, COLOR_PAIR(14));
    wattroff(pass, COLOR_PAIR(14));
    titlew = newwin(3, 70, 1, 1);
    wattrset(titlew, COLOR_PAIR(2));
    box(titlew, ACS_BULLET, ACS_BULLET); // sets default borders for the window
    mvwprintw(titlew, 1, 1, "%27sQUAN LY HO KHAU%28s", " ", " ");
    wattroff(titlew, COLOR_PAIR(2));
    wattroff(login, A_STANDOUT);
    wattron(login, COLOR_PAIR(16));
    mvwprintw(login, 10, 2, "Press any key except word, enter to EXIT");
    wattroff(login, COLOR_PAIR(16));
    wrefresh(titlew);
    wrefresh(login);
}
...
void dangNhap()
{
    ...
    do
    {
        ...
        echo();
        // ch = mvwgetch(id, 1, 11);
        ch = nhap(tai_khoan, "Tai Khoan: ", 1, 1, id);
        box(id, ACS_VLINE, ACS_HLINE);
        chuanHoaTen(tai_khoan);
        if (ch == 27)
            break;
        noecho();
        ch = password(mat_khau, "Mat Khau: ", 1, 1, pass);
        box(pass, ACS_VLINE, ACS_HLINE);
